...
public class simpleTestLesson6 {
    ...
    private Wait<WebDriver> wait;   // 4) на этом уровне я создам новую переменную
    private Wait<WebDriver> fluentWait; // 12) на этом уровне я создам новую переменную
...
//        принципе он работает абсолютно точно также как webdriver вы только у него другой синтаксис
    ...
    public void loginToKoel_correctCredentials() {
        // 5)код должен быть сухим давайте создадим новую переменную:
        By emailBy = By.xpath("//*[@type='email']");

        // 4) здесь я хочу ждать пока не появится e-mail и когда он только появится тогда я его сохраню.
        // wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@type='email']")));

        // 6) (explicit wait) и теперь вот этот e-mail я могу вот сюда отправить
        wait.until(ExpectedConditions.visibilityOfElementLocated(emailBy));

        // 7) и могу подправить его соответственно сюда:
        ...
        // 8) now we need to do it here (for not repeating the code)
    ...
    // 17) повторю вот для этого места
    ...
    public void loginToKoel_createPlaylist() {
        ...
        // 19) я вот этот ""//*[@class='fa fa-plus-circle create']"" вытащил отдельный класс:
        ...
        // 20) я вот этот "//div[@class='success show']"вытащил отдельный класс:
